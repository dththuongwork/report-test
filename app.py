...
# ---------------------------------------------------------
# CẤU HÌNH NHÓM KHU (MẶC ĐỊNH NẾU KHÔNG CÓ TRONG EXCEL)
# ---------------------------------------------------------
# Bạn có thể chỉnh sửa tên các khu tại đây (Ghi đúng chữ hoa/thường tuỳ ý)
NHOM_MAC_DINH = {
...
def clean_text(text):
    ...
    # Chuẩn hoá Unicode (khắc phục lỗi gõ tiếng Việt bị tàng hình)
...
@st.cache_data(ttl=86400*30) # Lưu cache 30 ngày
...
def get_week_label(date_obj):
    # Trả về chuỗi ngày T2 - CN của tuần đó
...
def process_data(df_kiot, df_har, map_kiot, map_har, loai_bo_qua):
    ...
    cols_map = ['Tên chuẩn', 'Khu', 'Loại']
    has_nhom = 'Nhóm' in df_map.columns
    if has_nhom: cols_map.append('Nhóm')

    df_merged = df_raw.merge(df_map[cols_map], on='Tên chuẩn', how='left')
    ...
    if has_nhom:
        df_merged['Nhóm'] = df_merged['Nhóm'].fillna('Khác').replace('', 'Khác')
...
def assign_group(row):
    # Nếu file Excel của người dùng tự tạo cột "Nhóm", dùng luôn!
    if 'Nhóm' in row and pd.notna(row['Nhóm']) and str(row['Nhóm']).strip() != 'Khác' and str(row['Nhóm']).strip() != '':
        return row['Nhóm']
    
    # Nếu không, dùng danh sách mặc định ở trên
    khu_name = clean_text(row['Khu'])
    for group_name, khu_list in NHOM_MAC_DINH.items():
        for k in khu_list:
...
st.sidebar.info("Dashboard tự động tải từ Link Google Drive công khai.")
...
# Nút xoá Cache
if st.sidebar.button("🔄 LÀM MỚI DỮ LIỆU (XOÁ CACHE)"):
    st.cache_data.clear()
    st.rerun()
