...
def normalize_cols(df):
    ...
    for target, fallbacks in targets.items():
        for f in fallbacks:
            if f in df.columns:
                new_cols[f] = target
                break # Chỉ đổi tên cột ĐẦU TIÊN tìm thấy, tránh bị trùng lặp tên cột
                
    df.rename(columns=new_cols, inplace=True)
    df = df.loc[:, ~df.columns.duplicated()] # Xoá các cột trùng lặp nếu có để tránh lỗi InvalidIndexError
    return df
...
def parse_money(val):
    ...
    val = str(val).strip()
    # Giữ lại các chữ số, dấu chấm, dấu phẩy, dấu âm (để loại bỏ ký hiệu tiền tệ ₫, VNĐ, đ...)
    val = re.sub(r'[^\d\.\,\-]', '', val)
...
def assign_group(row):
    ...
    # Ưu tiên 1: Tên Khu trùng hoặc là chuỗi con của tên Nhóm
    ...
    # Ưu tiên 2: Khớp 2 chiều (Khu nằm trong List hoặc List nằm trong Khu)
...
with tab_mapping:
    ...
    loai_options = ["Chưa phân loại", "Vé thường", "Vé đoàn", "Member", "Phụ kiện"]
