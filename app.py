...
def clean_text(text):
    ...
    text = unicodedata.normalize('NFC', tdef normalize_cols(df):
    if df.empty: return df
    df = df.copy()
    df.columns = [clean_text(c) for c in df.columns]
    
    targets = {
        'Tên sản phẩm': ['tên sản phẩm', 'tên hàng'],
        'SKU': ['sku', 'mã sản phẩm', 'mã hàng'],
        'Khu': ['khu', 'tên khu', 'khu game'],
        'Nhóm': ['nhóm'],
        'Loại': ['loại', 'phân loại'],
        'Ngày': ['ngày', 'thời gian', 'ngày tạo', 'created at', 'ngày đặt hàng', 'ngày bán', 'ngày lập', 'thời gian tạo'],
        'Số lượng': ['số lượng', 'số sản phẩm', 'sl', 'quantity', 'tổng số lượng'],
        'Thành tiền': ['thành tiền', 'doanh thu thuần', 'doanh thu chi tiết', 'doanh thu', 'tổng tiền', 'thực thu', 'tổng cộng', 'giá trị đơn hàng', 'giá'],
        'Trạng thái thanh toán': ['trạng thái thanh toán', 'trạng thái tt', 'payment status']
    }
    
    new_cols = {}
    for target, fallbacks in targets.items():
        matched = False
        for f in fallbacks:
            if f in df.columns:
                new_cols[f] = target
...
@st.cache_data(ttl=86400*30)
def process_data(df_kiot, df_har, map_kiot, map_har, haravan_status):
    ...
    if 'Ngày' in df_har.columns: df_har['Ngày'] = pd.to_datetime(df_har['Ngày'], dayfirst=False, errors='coerce')
    ...
    df_merged['Khu'] = df_merged['Khu'].fillna('').replace('Chưa phân loại', '')
    df_merged['Loại'] = df_merged['Loại'].fillna('').replace('Chưa phân loại', '')
...
def assign_group(row):
    ...
    if not khu_name: return "Khu Khác"
    
    for group_name in nhom_dict.keys():
        g_clean = clean_text(group_name)
        if g_clean == khu_name or khu_name in g_clean or g_clean in khu_name:
            return group_name
            
    for group_name, khu_list in nhom_dict.items():
        for k in khu_list:
            k_clean = clean_text(k)
            if k_clean in khu_name or khu_name in k_clean:
                return group_name
    return "Khu Khác"
...
with st.spinner("Đang tải dữ liệu..."):
    ...
    df_report, df_mapping_status = process_data(df_kiot, df_har, map_kiot, map_har, haravan_status)
...
with tab_mapping:
    ...
    khu_options = [""] + list(current_nhom.keys()) + [k for khus in current_nhom.values() for k in khus]
    khu_options = list(dict.fromkeys(khu_options)) 
    loai_options = ["", "Vé thường", "Vé đoàn", "Member", "Phụ kiện"]
    ...
    def render_mapping_table(title, source, col_obj, unique_key):
        with col_obj:
            st.markdown(f"## {title}")
            only_missing = st.checkbox(f"Chỉ hiện SP trống Khu/Loại", value=True, key=f"chk_{unique_key}")
            ...
            if only_missing:
                df_src = df_src[(df_src['Khu'] == '') | (df_src['Loại'] == '')]
...
with tab_dashboard:
    if not df_report.empty:
        ...
        if "" in all_loai: all_loai.remove("")
        ...
        for i, group_name in enumerate(groups):
            with tabs[i]:
                ...
                selected_khu = sorted(list(df_group['Khu'].unique()))
