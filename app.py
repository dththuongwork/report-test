...
# Cấu hình mặc định nếu file chưa tồn tại
NHOM_MAC_DINH = {
...
def process_data(df_kiot, df_har, map_kiot, map_har, loai_bo_qua, haravan_status):
    ...
    # BỘ LỌC HARAVAN (KHẮC PHỤC LỖI TRỐNG DATA)
...
with st.spinner("Đang tải dữ liệu..."):
    ...
    df_report, missing_map = process_data(df_kiot, df_har, map_kiot, map_har, loai_bo_qua, haravan_status)
...
with tab_mapping:
    ...
    st.info("Chỉnh sửa nhóm tuỳ ý. Bạn có thể thêm Nhóm hoặc sửa Khu. Bấm 'Lưu' khi hoàn tất.")
    ...
    if st.button("💾 Lưu thay đổi Nhóm"):
        ...
        if push_to_github_if_configured(NHOM_FILE): st.success("Đã lưu Nhóm vĩnh viễn lên GitHub!")
        else: st.warning("Đã lưu tạm trên máy chủ. Hãy tải file Backup đề phòng server reset.")
        st.cache_data.clear()
    
    if os.path.exists(NHOM_FILE):
        with open(NHOM_FILE, 'r', encoding='utf-8') as f:
            st.download_button("📥 Tải file Backup Nhóm", f, file_name=NHOM_FILE)
    
    st.divider()
    
    st.markdown("## 2. Phân loại Sản phẩm mới")
    if missing_map.empty:
        st.success("Tuyệt vời! Toàn bộ sản phẩm đã được phân loại.")
    else:
        st.warning("Các sản phẩm dưới đây chưa có Khu. Vui lòng gõ tên Khu và Loại vào cột bên cạnh.")
        df_edit = missing_map[['Tên gốc']].drop_duplicates().copy()
        df_edit['Khu mới'] = ""
        df_edit['Loại mới'] = ""
        
        edited_df = st.data_editor(df_edit, key="mapping_editor", use_container_width=True)
        if st.button("💾 Lưu các phân loại mới"):
            changed = edited_df[(edited_df['Khu mới'] != "") | (edited_df['Loại mới'] != "")]
            if not changed.empty:
                ...
                for _, row in changed.iterrows():
                    ...
                    current_mapping[ten_chuan] = {
                        ...
                        "Khu": row['Khu mới'] if row['Khu mới'] else "Chưa phân loại", 
                        "Loại": row['Loại mới'] if row['Loại mới'] else "Chưa phân loại"
                    }
                save_json(MAPPING_FILE, current_mapping)
                if push_to_github_if_configured(MAPPING_FILE): st.success("Đã lưu Sản phẩm vĩnh viễn lên GitHub!")
                else: st.warning("Đã lưu tạm trên máy chủ. Hãy tải file Backup đề phòng server reset.")
                st.cache_data.clear()
                st.rerun()
    ...
    if os.path.exists(MAPPING_FILE):
        with open(MAPPING_FILE, 'r', encoding='utf-8') as f:
            st.download_button("📥 Tải file Backup Sản phẩm", f, file_name=MAPPING_FILE)
